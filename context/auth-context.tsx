...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        // Check local demo auth first
        if (typeof window !== 'undefined' && localStorage.getItem('demo_logged_in') === 'true') {
            const demoUser = { id: 'demo-user-123', email: 'demo@studiox.com' } as any as User;
            setUser(demoUser);
            setSession({ user: demoUser, access_token: 'demo-token' } as any as Session);
            fetchCredits(demoUser.id);
            setLoading(false);

            // Still set up the auth listener for other potential changes, but mock its behavior for demo
            const {
                data: { subscription },
            } = supabase.auth.onAuthStateChange((_event, session) => {
                // Ignore real auth state changes if demo logged in
            });
    ...
    const signInWithEmail = async (email: string, pass: string) => {
        if (email === "demo@studiox.com" && pass === "DemoPassword123!") {
            const demoUser = { id: 'demo-user-123', email: 'demo@studiox.com' } as any as User;
            const demoSession = { user: demoUser, access_token: 'demo-token' } as any as Session;
